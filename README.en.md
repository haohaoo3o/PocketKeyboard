...
- **Broadcast name distinct from the phone**: on launch the app renames the local Bluetooth device to `PocketKeyboard_<brand>` (adapted from the system brand, e.g. `PocketKeyboard_Huawei` / `PocketKeyboard_Redmi`) so hosts can tell this is PocketKeyboard, not the phone itself
